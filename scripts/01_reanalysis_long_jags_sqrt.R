...
model{
  ...
  for(ii in 1:SiYr){
    ...
    hug_p_mu[ii] <- mu_hug_p + plot_hug_p[plot[ii]] + year_hug_p[year[ii]]
    
    ### note change for site type
    occ_p_mu[ii] <- mu_occ_p + beta_type_occ_p*(site_type[ii]-1) + plot_occ_p[plot[ii]] + year_occ_p[year[ii]]
    psi_mu[ii] <- mu_psi + beta_type_psi*(site_type[ii]-1) + plot_psi[plot[ii]] + year_psi[year[ii]]
    ...
    ## Huggins model
    for(jj in 1:t_max){
      p[ii,jj] <- 1-(1-hug_p[ii])^t[ii,jj]
      q[ii,jj] <- 1- p[ii,jj]
      hug_likeA[ii,jj] <- ifelse(t[ii,jj]>0, p[ii,jj]^n_cap[ii,jj]*(1-p[ii,jj])^(Mt1[ii]-n_cap[ii,jj]), 1.0)
    }
    p_star[ii]<-1-prod(q[ii,])
    ...
    hug_like[ii]<-prod(hug_likeA[ii,])/(p_star[ii]^(Mt1[ii]))
    hug_ones[ii]~dbern(hug_like[ii])
  }
