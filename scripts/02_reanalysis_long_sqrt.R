...
dh<-suppressWarnings(
  pblapply(1:SiYr,function(ii){
    temp<-dh_dat_all_PPM_4 %>% filter(Year==SiYr_all$Year[ii],
...
ch<- pblapply(1:nrow(SiYr_all),function(ii){ 
  ...
  temp <- CR_tab2 %>% filter(Year==SiYr_all$Year[ii],
                           Site==SiYr_all$Site[ii],
                           `100mPlot`==SiYr_all$`100mPlot`[ii]) %>%
    select(Unique_Indiv_ID, all_of(as.character(1:4))) %>% as.data.frame()
  # temp<-Filter(function(x) !all(is.na(x)),temp)
  return(temp)
})
...
est <- est %>% 
  ...
  pivot_wider(id_cols = c(Site, `100mPlot`, Year), 
              names_from = name, 
              values_from = 5:13, 
